...
new
    ...
    public function getWhatsAppLink(Order $order)
    {
        ...
        $message = urlencode("Hola {$order->customer_name}, te escribo de Monte Carmelo por tu pedido #{$order->id}.");
...
<div>
    ...
    <div class="mb-8">
        ...
        <p class="text-text-muted mt-1 font-light tracking-wide">Monitorea tus oportunidades de venta en tiempo real.
        </p>
    </div>
    ...
    <div class="flex flex-col md:flex-row gap-4 mb-6 items-center justify-between">
        <div class="w-full md:w-1/3 relative">
            ...
            <input type="text" wire:model.live.debounce="search" placeholder="Buscar cliente..."
        ...
        <select wire:model.live="statusFilter"
            class="bg-[#121212]/50 border border-white/10 rounded-full py-2 px-4 text-white focus:border-primary focus:ring-0 cursor-pointer w-full md:w-auto">
    ...
    <div class="glass-panel p-1 rounded-xl overflow-hidden">
        <x-mary-table :rows="$orders" :headers="[
        ['key' => 'created_at', 'label' => 'Fecha'],
        ['key' => 'customer_name', 'label' => 'Cliente'],
        ['key' => 'total', 'label' => 'Total'],
        ['key' => 'status', 'label' => 'Estado'],
        ['key' => 'actions', 'label' => 'Acciones']
    ]" class="table-glass" with-pagination>
            ...
            @scope('status', $order)
            <div
                class="px-3 py-1 rounded-full text-[10px] uppercase font-bold tracking-wider inline-flex items-center gap-1 {{ $this->getStatusClasses($order->status) }}">
            ...
            <div class="flex gap-2 justify-end">
                <x-mary-button icon="o-chat-bubble-left-right" link="{{ $this->getWhatsAppLink($order) }}" external
                    class="btn-ghost btn-sm text-success hover:bg-success/10" tooltip="WhatsApp" />
                <x-mary-button icon="o-eye" wire:click="show({{ $order->id }})"
                    class="btn-ghost btn-sm text-primary hover:bg-primary/10" tooltip="Ver Detalles" />
            </div>
            @endscope
        </x-mary-table>
    </div>
    ...
    {{-- DRAWER DETAILS --}}
    <x-mary-drawer wire:model="drawer" title="" right
        class="w-11/12 lg:w-1/3 glass-dark !bg-[#121212]/90 backdrop-blur-xl border-l border-white/10">
        @if($editingOrder->id)
            <div class="pt-6 space-y-8">
                ...
                <div class="flex items-center justify-between border-b border-white/10 pb-4">
                    ...
                    <div
                        class="px-3 py-1 rounded-full text-xs font-bold border {{ $this->getStatusClasses($editingOrder->status) }}">
                    ...
                    <div class="glass-panel p-3 rounded-lg border border-white/5">
                        ...
                        <div class="flex items-center justify-between text-sm text-white">
                            <div class="flex items-center gap-2">
                                ...
                                {{ $editingOrder->customer_phone ?? 'Sin teléfono' }}
                            </div>
                            <x-mary-button icon="o-chat-bubble-left-right"
                                link="{{ $this->getWhatsAppLink($editingOrder) }}" external
                                class="btn-circle btn-xs btn-success text-white" tooltip="Enviar WhatsApp" />
                        </div>
                    </div>
                    <div class="glass-panel p-3 rounded-lg border border-white/5">
                        ...
                        <div class="flex items-center gap-2 text-sm text-white">
                            ...
                            {{ $editingOrder->created_at->format('d/m/Y') }}
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white/5 p-4 rounded-xl border border-white/5">
                    ...
                    <x-mary-form wire:submit="save">
                        <x-mary-select label="Actualizar Estado" :options="[
                ...
                ['id' => 'cancelled', 'name' => 'Cancelado']
                        ...
                        <x-mary-textarea label="Notas Internas" wire:model="notes"
                            placeholder="Detalles de entrega, pagos, etc..." rows="3"
                            class="bg-[#121212] border-white/10 text-sm mt-2" />

                        <div class="flex justify-end mt-4">
                            <x-mary-button label="Guardar Cambios"
                                class="btn-primary btn-sm w-full font-serif uppercase tracking-widest" type="submit"
                                spinner="save" />
                        </div>
                    </x-mary-form>
                </div>
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        @foreach($editingOrder->items as $item)
                            <div
                                class="flex items-start gap-4 p-3 rounded-xl bg-white/5 border border-white/5 hover:border-primary/30 transition-colors">
                                <div
                                    class="w-12 h-12 bg-[#121212] rounded-lg flex items-center justify-center text-primary border border-white/5">
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="text-xs text-text-muted mt-1">
                                        {{ $item['qty'] }}
                                        @if(isset($item['unit_type']) && $item['unit_type'] == 'kg') kg
                                        @elseif(isset($item['unit_type']) && $item['unit_type'] == 'unit') unidades @endif
                    ...
                    <div
                        class="flex justify-between items-center mt-8 p-4 bg-primary/10 rounded-xl border border-primary/20">
